...
class AnchorDetail(BaseModel):
    ...
    end_line: Optional[int] = Field(None, description="1-indexed end line (optional).")
    excerpt: str = Field("", description="Source text excerpt (secret-masked).")
...
class FindingSummary(BaseModel):
    ...
    title: str
    anchor: Optional[AnchorDetail] = None
    source: str = Field(
...
class CategoryScoreItem(BaseModel):
    ...
    earned: float = Field(..., ge=0.0, description="Weighted credit earned.")
    possible: float = Field(..., ge=0.0, description="Maximum possible credit (excludes NOT_ASSESSABLE).")
    excluded_count: int = Field(..., ge=0, description="NOT_ASSESSABLE controls excluded.")
...
class CoverageLimitationItem(BaseModel):
    ...
    The controls listed in affected_control_ids were evaluated as NOT_ASSESSABLE
    because this fragment could not be resolved.  Grade language describes
    assessed-control coverage only; these gaps are not treated as missing.
    """
    ...
    reason: str = Field(
        ..., description="Human-readable explanation of why this fragment was unresolved."
    )
    affected_control_ids: list[str] = Field(
        default_factory=list,
        description="Catalogue control IDs rendered NOT_ASSESSABLE by this limitation.",
    )
...
class HumanReviewItem(BaseModel):
    ...
    finding_id: Optional[uuid.UUID] = Field(
        None, description="Finding ID if this is an AI advisory finding."
    )
    control_id: str = Field(
...
class AnalysisReport(BaseModel):
    ...
    The advisory_disclaimer field is required and non-nullable; model validation
    rejects blank or missing values.

    BR-02 compliance: no field or string in this model asserts that a pipeline
    is secure, compliant, or certified.  Grade language describes coverage of
    ...
    format: str = Field(
        ..., description="Detected pipeline format: github_actions, gitlab_ci, jenkins."
    )
    ...
    total_score: Optional[float] = Field(
        None,
        description=(
            "Weighted 0-100 score representing coverage of assessed controls. "
            "Null when all controls are NOT_ASSESSABLE (unscorable)."
        ),
    )
    ...
    unscorable_reason: Optional[str] = Field(
        None,
        description="Reason scoring could not be performed (e.g. 'all_not_assessable').",
    )

    # Category breakdown
    category_scores: list[CategoryScoreItem] = Field(
        default_factory=list,
        description="Per-category scoring breakdown sorted by category identifier.",
    )

    # Severity distribution (deterministic findings only)
    ...
    coverage_limitations: list[CoverageLimitationItem] = Field(
        default_factory=list,
        description=(
            "Unresolved fragments that caused controls to be NOT_ASSESSABLE. "
            "Detection coverage is per-format so Jenkins gaps are not hidden "
